...
float ADD_BUILTIN_PREFIX(logf)(float x)
{
   ...
   s = (fpX >> 31) & 1;				//sign 
   e = SELECT_RANGE(fpX, 30, 23);	//exponent
   m = fpX & 0x007fffff;			//matissa - fraction part - 32bit && with 0s and 1s to extract fraction part
   ...
   if(e == 0)
   {
      ...
      e = -subnormal_lz;
      m = SELECT_RANGE(mshifted, 21, 0) << 1;  // shift right by one
   }
#endif
   ...
   cmpSqrt2 = (m >> 17) >= sqrt2;   //boolen expression
   // m is int, which is 4bits only, why are we shift it by 17?
   // sqrt is char=26, in binary =  00110010 00110110 ?
